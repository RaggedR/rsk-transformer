...
\begin{abstract}
We train an encoder-only transformer to learn inverse combinatorial bijections: the Robinson--Schensted--Knuth (RSK) correspondence, the Hillman--Grassl correspondence for reverse plane partitions, and the cylindric growth diagram bijection for cylindric plane partitions. The key idea is to encode each tableau entry as a token with four learned embeddings (value, row, column, tableau identity), preserving 2D geometric structure. The same architecture handles all tasks without modification. On $n=10$ permutations, the model achieves \textbf{100\% exact-match accuracy} on 50,000 held-out test samples, having been trained on only 14\% of the permutation space with 1.2M parameters---ruling out memorisation. On $n=15$ (1.3 trillion permutations), training on 0.00004\% of the space yields 99.99\% exact match. The model also achieves 100\% on reverse plane partitions and cylindric plane partitions, the latter requiring inversion of a recursive growth diagram process with no closed-form algorithm. This significantly improves on the PNNL ML4AlgComb benchmark, which achieved only weak baselines using flat bracket-string encoding.
\end{abstract}
...
\section{Experiment 3: Reverse Plane Partitions (Hillman--Grassl)}
...
\section{Experiment 4: Cylindric Plane Partitions}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Larger cylindric profiles}: scale the cylindric growth diagram experiments to longer profiles ($T{=}8,10$) and higher label values.
    \item \textbf{Cylindric RSK}: Dobner~\cite{dobner2026} defined an RSK analogue for cylindric tableaux, directly relevant to the growth diagram framework in~\cite{langer2013}. Combining this with the cylindric growth diagram approach is a natural next step.
\end{itemize}
